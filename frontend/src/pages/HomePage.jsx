  ...
  return (
    <Box sx={{width: '100%', px: {xs:'2%', sm:'2%', md:'2%', lg: '8%'}, backgroundColor: '#000'}}>
      ...
      <Box sx={{ py: 8, width: '100%'}}>
        <Box display="flex" flexDirection={{ xs: 'column', md: 'row' }} gap={8} alignItems="center" justifyItems={'space-between'}>
        <CardMedia
            component="img"
            image= {historyImage}
            alt="Our History"
            sx={{ borderRadius: 5, width: { xs: '100%', md: '45%' }, height: 'auto' }}
          />
        <Box>
            <Typography variant="h3" gutterBottom color='#fff'>Our History</Typography>
...
Over the years, El Amigo has become more than a restaurant, it's a gathering place for friends, families, and anyone craving that warmth only great food can bring.
      ...
      <Box sx={{ py: 8, width: '100%'}}>
        <Box display="flex" flexDirection={{ xs: 'column', md: 'row' }} gap={8} alignItems="center" justifyItems={'space-between'}>
        <Box>
            <Typography variant="h3" gutterBottom color='#fff'>Word from the Chef</Typography>
            <Typography variant="body1" color='#fff'>
...
Whether you're enjoying our hearty tortas or a plate of hot tamales, we’re honored to share a taste of Mexico with you. Thanks for being part of the El Amigo family.
